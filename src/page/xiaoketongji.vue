      ...
      <div class="info">
        <van-cell-group>
          ...
          <van-dropdown-menu active-color="#1989fa">
            <van-dropdown-item v-model="TeacherID" :options="option1" v-if="isTeacher == 0"/>
            <van-dropdown-item v-model="CourseID" :options="option2" />
          </van-dropdown-menu>
      </div>
      <div class="sButton" @click="getListTerCourseRowClass">
        搜索
      </div>

    </div>
    <div class="banjiListItem" v-for="(item,index) in banjilist">
      <div><span class="title">老师名称：</span><span class="">{{item.Name}}</span></div>
      <div><span class="title">老师手机：</span><span class="">{{item.Phone}}</span></div>
      <div><span class="title">课程名称：</span><span class="">{{item.CourseName}}</span></div>
      <div><span class="title">授课课时：</span><span class="">{{item.allcount}}</span></div>
      <div><span class="title">授课总金额：</span><span class="">{{item.sumAmount}}</span></div>
    </div>
...
<script>
  import { getListTerCourseRowClass,getListTeacher,getwxterlistCourse } from "@/api/getData";
  ...
  export default {
    ...
    created() {
      ...
      this.getListTerCourseRowClass();
      this.getListTeacher();
      this.getwxterlistCourse();
    },
    ...
    methods: {
      ...
      async getwxterlistCourse(){
        const crs = await getwxterlistCourse({

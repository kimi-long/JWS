...
<script>
  import { WXUploadimg,stuGetListStoresName,getwxListRole,getwxTeacherInfo,editTeacher } from "@/api/getData";
    ...
    data() {
      return {
        ruleForm:{
                ...
                teaID:localStorage.getItem("teaID")
         ...
         this.getwxListRole();
          ...
          this.$store.commit('fullLoadingFun',false);
            ...
            const crs = await getwxListRole({
                ...
                const crs = await getwxTeacherInfo({
                ...
                if(crs.orsuccess == '1'){
                    ...
                    var str = crs.data.Role;
                var arr = str.split(',');
                 this.checkedRoles = arr;
                    this.currentBaseImg = crs.data.HeadUrl;
                         this.$store.commit('fullLoadingFun',false);
                ...
                this.ruleForm.TeacherID = this.teaID;
                const crs = await editTeacher(this.ruleForm)
                // debugger
                // console.log(crs)
                if(crs.orsuccess == '1'){
